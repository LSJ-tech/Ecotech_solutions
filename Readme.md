...
- Usuarios con nombre vacío o contraseña que no cumple la política (mínimo 8 caracteres con letras y números).
...
| 3.1.2 Autenticación y validación de entradas | Login con PBKDF2, política de contraseñas (8+ caracteres, letras y números) y roles; códigos de rol y llave de API en `.env`; `validar_ciudad()`, lista blanca de indicadores, `validar_monto()`; opciones filtradas por rol y verificadas al ejecutar. |
...
Las contraseñas y códigos secretos se muestran como asteriscos mientras se escriben. Al crear una cuenta la contraseña debe tener al menos 8 caracteres y combinar letras y números (`validar_contrasena()`); la política se aplica también en `generar_hash_contrasena()`, por donde pasa toda contraseña persistida, y en `Usuario.actualizar_contrasena()`. Los códigos secretos de rol no están sujetos a ella, porque son secretos de configuración y no credenciales de usuario. Las contraseñas se almacenan mediante PBKDF2 y no en texto plano.
